...
- **`meta.ts` refines one folder** (`defineMeta({ title, icon, order, collapsed, pages })`). The `pages` array lists children by slug (numeric prefix and parentheses stripped); **children you omit sort _after_ the listed ones** — so list `"index"` first when you set `pages`, or the folder's landing page (the tab's target) sinks to the bottom.
- **Sidebar render mode is global, not per-folder.** `navigation.sidebar.display` in `blume.config.ts` is `"flat"` (default), `"group"` (collapsible), or `"page"` (drill-in sub-panel) and applies to **every** group at once. (It used to live on each folder's `meta.ts` as `display`; that field was removed in 0.6.0 — set it once in config instead.) An explicit `navigation.sidebar` item may still override its own group's `display`.
...
- **`theme`:** `accent` (a color string for both modes, or `{ light, dark }` per mode), `action` (color), `mode` (`light`/`dark`/`system`), `strict`, `radius`, `fonts` (`{ body, display }` — curated Google-font slugs), `background` and `backgroundImage` (each a string, or `{ light, dark }` per mode), `css`. The old `accentDark`/`backgroundDark`/`backgroundImageDark` fields were **merged into these per-mode objects** — a bare string still applies to both modes, so only reach for `{ light, dark }` when the two modes differ.
